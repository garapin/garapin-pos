...
part 'report_transaction_bagi_bagi_state.dart';

part 'report_transaction_bagi_bagi_cubit.freezed.dart';

class ReportTransactionBagiBagiCubit extends BaseCubit<ReportTransactionBagiBagiState> {
  ReportTransactionBagiBagiCubit(BuildContext context)
      : super(context, const ReportTransactionBagiBagiState());

  final FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin =
  FlutterLocalNotificationsPlugin();

  void getData(String startDate, String endDate, {String filter = ""}) async {
    final data = await ApiService.getReportTransactionBagiBagi(
      context,
      startDate: startDate,
      endDate: endDate,
  ...
  void clearTransaction() {
    emit(state.copyWith(transaction: ReportTransactionBagiBagi()));
  }
  ...
  Future<String> saveToExcel(String bufferExcel) async {
    ...
    print(dir);
    File file = File(
        "$dir/""report_transaction_bagi_bagi_${DateTime.now().millisecondsSinceEpoch}.xlsx");
    await file.writeAsBytes(bytes);
    showInfo("Export excel to Download Folder");
    await showNotification("Success", "Exported Report Bagi Bagi");
    return file.path;
  }
